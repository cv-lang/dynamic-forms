...
namespace Cvl.DynamicForms.Infrastructure.Importers.Excel.Helpers
{
    ...
    public class ExcelRowReader
    {
        ...
        public ControlDescription ReadControlRow(IWorksheet ws, int row, int level)
        {
            ...
            {
                Row = row,
                ElementName = ws.GetCellText(row, ExcelColumnIndex.ElementName) ?? throw new Exception($"Brak ElementName row:{row}"),
                ElementType = ws.GetCellText(row, ExcelColumnIndex.ElementType)?.Trim() ?? "auto",
